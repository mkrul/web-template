...
export const validFilterParams = (
  ...
  const { listingFieldsConfig, defaultFiltersConfig, listingCategories } =
    filterConfigs;
...
export const initialValues =
  (props, currentQueryParams) => (queryParamNames, isLiveEdit) => {
...
export const pickSearchParamsOnly = (
  ...
  // When origin is used, bounds may be programmatically expanded and may not match URL exactly.
  // Ignore bounds in comparison to avoid perpetual "loading" visuals.
  const boundsMaybe = isOriginInUse && origin ? {} : bounds ? { bounds } : {};
