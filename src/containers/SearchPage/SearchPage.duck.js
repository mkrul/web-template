...
// Debug logging helper for search flows
const DEBUG_SEARCH_LOGS =
  process.env.NODE_ENV !== 'production' &&
  process.env.REACT_APP_DEBUG_SEARCH === 'true';
const debugLog = (...args) => {
  if (DEBUG_SEARCH_LOGS) console.log(...args);
};
const debugError = (...args) => {
  if (DEBUG_SEARCH_LOGS) console.error(...args);
};
...
export const setDeliveryAddress = (address) => (dispatch, getState, sdk) => {
  ...
  return sdk.currentUser
    ...
    .catch((e) => {
      debugError('[Search] setDeliveryAddress error:', e);
...
export const searchListings =
  (searchParams, config) => (dispatch, getState, sdk) => {
    ...
    // eslint-disable-next-line no-console
    debugLog('[Search] Incoming searchParams:', searchParams);
    ...
    // eslint-disable-next-line no-console
    debugLog('[Search] sdk.listings.query params:', params);
    ...
    return sdk.listings
      ...
      .catch((e) => {
        ...
        // eslint-disable-next-line no-console
        debugError('[Search] Query failed:', e);
...
export const loadData =
  (params, search, config) => (dispatch, getState, sdk) => {
    ...
    // If user searched by address (origin present), expand bounds to ~100-mile radius
    const RADIUS_METERS_100_MILES = 160934;
    const expandBoundsAround = (center, radiusMeters) => {
      ...
      const latDelta = toDeg(radiusKm / earthRadiusKm);
      const lngDelta = toDeg(radiusKm / (earthRadiusKm * Math.cos(toRad(lat))));
      return {
    ...
    const boundsExpandedMaybe = origin
      ? (() => {
          const expanded = expandBoundsAround(origin, RADIUS_METERS_100_MILES);
          if (!expanded) return { bounds: null };
          const ne = new sdkTypes.LatLng(expanded.ne.lat, expanded.ne.lng);
          const sw = new sdkTypes.LatLng(expanded.sw.lat, expanded.sw.lng);
          return { bounds: new sdkTypes.LatLngBounds(ne, sw) };
        })()
      : {};

    // eslint-disable-next-line no-console
    debugLog('[Search] loadData parsed:', {
      page,
      address,
      origin,
      rest,
      boundsExpanded: boundsExpandedMaybe?.bounds,
    });
